...
# Settings: static binary, large file support, and selected applets
# Use sed to replace in-place to avoid duplicate entries (kconfig keeps first value)
for bb_opt in LFS STATIC CAT CP DF ECHO LS MKDIR MV RM SYNC TEST TEST1 TEST2 CLEAR VI INIT MDEV MOUNT FEATURE_MOUNT_FLAGS UMOUNT ASH ASH_OPTIMIZE_FOR_SIZE ASH_ALIAS; do
...
yes "" | make ARCH=x86 oldconfig || error_exit "BusyBox oldconfig failed"
